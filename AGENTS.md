...
- Keep the webhook, rendering, queueing, and printing path explicit. Configuration is validated before any listener or printer work starts.
- Label rendering is deterministic. Exercise layout changes with focused tests and `mise run preview`.
- Tests use local HTTP servers and printer fakes; queue order, shutdown, and backpressure are behavioural contracts.
